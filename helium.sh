...
function install() {
    ...
    wget -q -O /etc/dnsmasq.conf "https://raw.githubusercontent.com/vinstechmy/AdsBlock/main/dnsmasq.conf"
    sed -i "s/YourPublicIP/${publicIP}/" /etc/dnsmasq.conf
    wget -q -O ${providers} "https://raw.githubusercontent.com/vinstechmy/AdsBlock/main/providers.txt"
    > ${tempHostsList}
